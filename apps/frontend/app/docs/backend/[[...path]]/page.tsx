...
/**
 * Serves the backend TypeDoc documentation
 * Handles all static files from the generated backend docs directory
 */
export default async function BackendDocsPage({ params }: PageProps) {
  ...
  if (!fs.existsSync(fullPath)) {
    ...
    if (fs.existsSync(backendDocsPath) && fs.statSync(backendDocsPath).isDirectory()) {
      const indexPath = path.join(backendDocsPath, 'index.html');
      if (fs.existsSync(indexPath)) {
        redirect('/docs/backend/index.html');
      }
    }
    notFound();
  }
  ...
  if (stats.isDirectory()) {
    ...
    if (fs.existsSync(indexPath)) {
      redirect(`/docs/backend/${requestedPath}/index.html`);
    }
    notFound();
  }
  ...
  if (ext !== '.html') {
    ...
    redirect(`/api/docs/backend/${requestedPath}`);
  }
