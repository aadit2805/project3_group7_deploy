...
export default function KitchenMonitor() {
  ...
  const fetchWeather = useCallback(async (city: string = 'College Station') => {
    try {
      ...
      const response = await fetch(
        `${backendUrl}/api/weather/current?city=${encodeURIComponent(city)}`,
        {
          credentials: 'include',
        }
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 animate-fade-in">
      ...
      <div className="mb-4 flex items-center justify-between animate-slide-in-down">
        ...
        <div className="relative">
          ...
          >
            ...
            <svg
              className={`w-4 h-4 transition-transform duration-200 ${isWeatherDropdownOpen ? 'rotate-180' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {isWeatherDropdownOpen && (
            <>
              <div className="fixed inset-0 z-10" onClick={() => setIsWeatherDropdownOpen(false)} />
      ...
      {/* Orders Grid */}
      <div
        className={`grid gap-4 ${isCompactView ? 'grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5'}`}
      >
        {orders.length === 0 ? (
          ...
          orders.map((order, index) => (
            ...
            >
              {/* Order Header */}
              <div
                className={`text-white ${isCompactView ? 'p-2' : 'p-3'} rounded-t-lg ${order.rush_order ? 'bg-red-600' : 'bg-gray-800'}`}
              >
                <h2 className={`font-bold ${isCompactView ? 'text-sm' : 'text-lg'}`}>
                  Order #{order.order_id}
                </h2>
                {order.rush_order && (
                  <p
                    className={`${isCompactView ? 'text-sm' : 'text-lg'} font-bold text-yellow-300 ${isCompactView ? 'mb-0' : 'mb-1'}`}
                  >
                    RUSH ORDER
                  </p>
                )}
              ...
              {order.order_notes && (
                <div
                  className={`bg-yellow-50 border-l-4 border-yellow-400 ${isCompactView ? 'p-2 mx-2 mt-2' : 'p-3 mx-3 mt-3'} rounded`}
                >
                  <p
                    className={`${isCompactView ? 'text-xs' : 'text-xs'} font-semibold text-yellow-800 ${isCompactView ? 'mb-0.5' : 'mb-1'}`}
                  >
                    ORDER NOTES:
                  </p>
                  <p
                    className={`${isCompactView ? 'text-xs' : 'text-sm'} text-yellow-900 whitespace-pre-wrap`}
                  >
                    {order.order_notes}
                  </p>
                </div>
              ...
              {/* Order Items */}
              <div
                className={`flex-1 ${isCompactView ? 'p-2 space-y-1.5' : 'p-4 space-y-3'} overflow-y-auto`}
              >
                {order.meals.map((meal, mealIdx) => (
                  <div
                    key={meal.meal_id}
                    className={`border-b ${isCompactView ? 'pb-1' : 'pb-2'} last:border-b-0`}
                  >
                    <p
                      className={`font-semibold ${isCompactView ? 'text-xs' : 'text-sm'} text-gray-700 ${isCompactView ? 'mb-0.5' : 'mb-1'}`}
                    >
                    ...
                    <ul className={`${isCompactView ? 'ml-2 space-y-0.5' : 'ml-3 space-y-1'}`}>
                      {meal.items.map((item, itemIdx) => (
                        <li
                          key={itemIdx}
                          className={`${isCompactView ? 'text-xs' : 'text-sm'} text-gray-600 flex items-start`}
                        >
